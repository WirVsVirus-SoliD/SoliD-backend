...
public interface OffersRepository extends JpaRepository<OfferEntity, Integer> {
	...
	@Query(value = "select oe.* from solid_offers oe where oe.user_t_id = ?1 and oe.provider_t_id=?2", nativeQuery = true)
	public OfferEntity findByUserAndProvider(int userId, int providerId);

	@Query(value = "select oe.* from solid_offers oe where oe.user_t_id = ?1", nativeQuery = true)
	public List<OfferEntity> findByUserId(int userId);

	@Query(value = "select oe.* from solid_offers oe where oe.provider_t_id = ?1", nativeQuery = true)
