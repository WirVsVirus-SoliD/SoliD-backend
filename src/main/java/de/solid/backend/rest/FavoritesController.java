...
@OpenAPIDefinition(
    ...
    info = @Info(title = "Example API", version = "1.0.1",
        contact = @Contact(name = "Example API Support", url = "http://exampleurl.com/contact",
...
public class FavoritesController extends BaseController {
  ...
  public Response getFavorites() {
    ...
    if (helperEntity != null) {
      List<FavoriteEntity> entities =
          this.favoritesRepository.findByHelperId(helperEntity.getT_id());
      return HTTP_OK(entities.stream().map(entity -> new FavoriteResponseModel().fromEntity(entity))
  ...
  @Produces(MediaType.APPLICATION_JSON)
  @Transactional
  public Response markFavorite(@RequestBody FavoriteRequestModel favoriteRequestModel) {
    HelperEntity helperEntity = this.helpersRepository.findByEmail(email);
    if (helperEntity != null) {
      ProviderEntity providerEntity =
          this.providersRepository.findById(favoriteRequestModel.getProviderId());
